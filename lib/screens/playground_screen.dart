  ...
  Future<void> _send() async {
    ...
    if (_apiKey.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: const Text('Clé API manquante — configurez-la dans Chat Gemini'),
    ...
    try {
      ...
      final response = await http
          .post(
            ...
            body: jsonEncode({
              'system_instruction': {
                'parts': [
                  {'text': _systemController.text.trim()}
                ],
              },
              'contents': [
                {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          _promptTokens = usage?['promptTokenCount'] as int? ?? 0;
          _completionTokens = usage?['candidatesTokenCount'] as int? ?? 0;
          _totalTokens = usage?['totalTokenCount'] as int? ?? 0;
          _latencyMs = elapsed;
          _isLoading = false;
        });
      } else {
      ...
      children: [
        ...
        if (_showSystemPrompt) ...[
          ...
          TextField(
            ...
            maxLines: 4,
            style: TextStyle(
                color: context.isRose ? const Color(0xFF1A0050) : Colors.white,
                fontSize: 13,
        ...
        TextField(
          ...
          decoration: InputDecoration(
            ...
            hintStyle: TextStyle(
                color: context.isRose
                    ? const Color(0xFF1A0050).withValues(alpha: 0.4)
